...
export function boletaParaImprimir(sale, settings = {}, opciones = {}) {
  ...
  const arriba = opciones.arribaMm ?? 2;
  /* Con un alto fijo la boleta NO se mide sola. Es la salida para cuando el
     controlador rechaza una página a medida —Windows con estas térmicas suele
     tener una lista cerrada de tamaños— y en vez de imprimir contesta "no se
     puede imprimir". */
  const altoFijo = Number(opciones.altoMm) || 0;
...
<style>
  /* La página es exactamente el rollo. El alto se mide y se escribe abajo, ya
     ...
     al milímetro. */
  @page { size: ${anchoPapel}mm ${altoFijo || 80}mm; margin: 0; }
  ...
  * { box-sizing: border-box; }
  ...
  .negocio { font-weight: 700; font-size: 10pt; line-height: 1.15; }
  .chico { font-size: 7pt; }
  .logo { display: block; margin: 0 auto 0.5mm; max-width: ${Math.round(anchoUtil * 0.5)}mm; max-height: 8mm; object-fit: contain; }
  .raya { border-top: 1px dashed #000; margin: 1mm 0; }
  ...
  /* Un respiro chico antes del logo: sin nada, el primer renglón sale pegado
     al corte anterior. */
  .arriba { height: ${arriba}mm; }
</style></head>
<body>
  <div class="boleta">
  ...
  <script>
  /* Medirse y fijar el alto de la página.

     NO puede colgarse del evento "load": esta ventana se llena con
     document.write() y para cuando el guion existe, "load" muchas veces YA
     pasó — el oyente se registra tarde y no se dispara nunca. Cuando eso
     ocurre queda el alto de respaldo del CSS, la página sale mucho más larga
     que la boleta, y aparecen el encabezado y el pie del navegador. Fue
     exactamente lo que pasó en el mesón.

     Así que se corre de inmediato y, si todavía faltan imágenes, se vuelve a
     medir cuando terminen de cargar. */
  (function () {
    ...
    function ajustarEImprimir() {
      ...
      if (!${altoFijo || 0}) {
        try {
          var caja = document.querySelector(".boleta");
          var mm = Math.ceil(caja.getBoundingClientRect().height * 25.4 / 96) + 1;
          var hoja = document.createElement("style");
          hoja.textContent = "@page { size: ${anchoPapel}mm " + mm + "mm; margin: 0; }";
          document.head.appendChild(hoja);
        } catch (e) { /* si algo falla, queda el alto de respaldo */ }
      }
      setTimeout(function () { window.print(); }, 150);
    }

    /* Si hay logo, hay que esperarlo: una imagen a medio cargar mide cero y la
       boleta saldría más corta que el papel que necesita. */
    var img = document.querySelector(".boleta img");
    if (img && !img.complete) {
      img.addEventListener("load", ajustarEImprimir);
      img.addEventListener("error", ajustarEImprimir);
      setTimeout(ajustarEImprimir, 1500);   // por si la imagen nunca contesta
    } else {
      setTimeout(ajustarEImprimir, 50);
    }
  })();
  <\/script>
</body></html>`;
