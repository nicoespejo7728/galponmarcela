...
/* Dos códigos "iguales para el ojo y la pistola" pueden llegar como texto
   distinto: un envase con un código UPC-A de 12 dígitos se guarda a veces
   como EAN-13 de 13 (con un cero adelante que lo completa), y basta con que
   alguien lo vuelva a escribir o escanear sin ese cero para que, como texto,
   deje de calzar con lo que ya había — el sistema no lo reconocía como el
   mismo código y terminó creando un producto duplicado (septiembre de 2026,
   "Capuccino vainilla Marley": quedó una ficha con "0781159004025" y otra
   con "781159004025", cada una con su propio stock). Para DETECTAR un
   choque —nunca para guardar ni mostrar— se compara así: quitando ceros a
   la izquierda de un código puramente numérico. Los códigos propios
   (INT-..., con letras) se comparan tal cual, sin tocar. */
export function normalizarCodigoParaComparar(codigo) {
  const c = String(codigo || "").trim().toUpperCase();
  if (!c || !/^\d+$/.test(c)) return c;
  return c.replace(/^0+(?=\d)/, "");
}
