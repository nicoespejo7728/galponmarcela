/* La base de datos del propio computador.
   ...
   - `copias`: lo último que se leyó de la base —catálogo, precios, gente— para
     que el sistema abra aunque no haya con qué consultarla.
...
export const ALMACEN_COPIAS = "copias";
  ...
  if (!promesa) {
    promesa = new Promise((resolve) => {
      ...
      req.onupgradeneeded = () => {
        ...
        if (!bd.objectStoreNames.contains(ALMACEN_COPIAS)) {
          bd.createObjectStore(ALMACEN_COPIAS, { keyPath: "clave" });
        }
      };
