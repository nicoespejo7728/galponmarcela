...
/* Protección contra una baja masiva por accidente.
   ...
   "eliminados" (ver diferencias() más abajo) es cualquier producto que estaba
   en la última copia conocida y no viene en la lista que se está guardando
   ahora — y eso se toma como "lo sacaron del catálogo", así que se marca
   inactivo. El supuesto es que la pantalla siempre manda el catálogo
   COMPLETO. Si por lo que sea manda una copia incompleta —quedó atrasada,
   una sincronización se saltó un producto— el sistema no tiene forma de
   distinguir eso de una baja real, y apaga en silencio todo lo que falte.
   Así se dieron de baja solos dos productos en agosto de 2026 sin que nadie
   los hubiera tocado.

   Una baja de verdad, hecha por una persona desde la pantalla, es siempre de
   a un producto por vez — no existe un "eliminar varios". Por eso, pasado
   este número en una sola llamada, se corta con un error en vez de
   aplicarlas: mejor avisar altiro que vaciar el catálogo sin que nadie se dé
   cuenta hasta días después. */
const TOPE_BAJAS_IMPLICITAS = 3;
function bajasSeguras(eliminados) {
  if (eliminados.length > TOPE_BAJAS_IMPLICITAS) {
    throw new Error(
      `Se iba a dar de baja ${eliminados.length} productos de una sola vez ` +
      `(${eliminados.slice(0, 5).map((p) => p.name).join(", ")}${eliminados.length > 5 ? "…" : ""}), ` +
      `algo que nadie pide hacer así — probablemente la copia del catálogo usada para guardar estaba ` +
      `incompleta. Se canceló esa baja para no perder productos por error: recarga la pantalla e inténtalo de nuevo.`
    );
  }
  return eliminados;
}
...
export const productos = {
  ...
  async escribir(actual, anterior, opciones = {}) {
    ...
    for (const p of (opciones.sinBajas ? [] : bajasSeguras(eliminados))) {
