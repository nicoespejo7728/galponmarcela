    ...
    await enParalelo(cambiados, async ({ antes, ahora }) => {
      if (distintos(antes, ahora, CAMPOS_PRODUCTO)) {
        ...
        if (Object.keys(fila).length > 0) {
          ...
          // Quien recibió mercadería (o corrigió el precio) sabía que bajaba
          // el precio con stock del anterior todavía en pie, y confirmó en
          // pantalla que aceptaba esa pérdida a propósito (ver ReceivingView,
          // migración 0037). La fila de historial que el gatillo acaba de
          // insertar se marca aparte para que la regla del precio anterior
          // deje de protegerla — no se manda a mano ninguna columna en el
          // insert, es el propio gatillo el que crea la fila.
          if (fila.precio !== undefined && ahora.__omitirProteccionAnterior) {
            const { error: errOmitir } = await sb.rpc("marcar_precio_sin_proteccion", { p_producto_id: ahora.id });
            if (errOmitir) throw new Error(`No se pudo confirmar la baja de precio de "${ahora.name}": ${errOmitir.message}`);
          }
        }
      }
