...
import { normalizarCodigoParaComparar } from "@/lib/codigos-barra";
...
export const configuracion = {
  async leer(opciones = {}) {
    ...
    return {
      ...
      // Recargo por pagar cigarros con tarjeta, editable en Ajustes. Ver
      // sistema-ventas.jsx (RECARGO_TARJETA_PCT_POR_OMISION) — sin esta
      // columna el número se perdía en cada recarga y volvía siempre al 21%
      // de fábrica; quedó corregido en la migración 0038.
      recargoTarjetaCigarrosPct: num(data.recargo_tarjeta_cigarros_pct),
  ...
  async escribir(valor) {
    ...
    const fila = {
      ...
      recargo_tarjeta_cigarros_pct: num(valor.recargoTarjetaCigarrosPct) || 21,
    };
...
/* =====================================================================
   admin-notes  ↔  galpon.bitacora_admin

   Sección propia, solo para administradores (migración 0038, pedido de
   Fran, sept. 2026): "media planilla, medio block de notas" para llevar un
   registro interno de gastos diarios/semanales/mensuales/anuales a su
   manera. Se guarda tal cual — columnas y filas libres, sin estructura fija
   de "gasto"— porque el pedido es justamente que cada quien la use como
   quiera; nadie más que un administrador puede ni leerla ni escribirla, ni
   siquiera a nivel de base de datos (ver política bitacora_admin_solo_admin).

   Vive en su propia tabla y no toca producto, venta, movimiento ni ninguna
   otra — nada de lo que se anote acá puede alterar Finanzas, Inventario o
   Análisis. */
const PERIODOS_BITACORA = ["diario", "semanal", "mensual", "anual"];

export const bitacoraAdmin = {
  async leer() {
    const sb = obtenerCliente();
    const { data, error } = await sb.from("bitacora_admin").select("*");
    if (error) throw new Error(error.message);
    const porPeriodo = {};
    for (const periodo of PERIODOS_BITACORA) {
      const fila = (data || []).find(f => f.periodo === periodo);
      porPeriodo[periodo] = {
        columns: Array.isArray(fila?.columnas) ? fila.columnas : [],
        rows: Array.isArray(fila?.filas) ? fila.filas : [],
        notes: fila?.notas || "",
      };
    }
    return porPeriodo;
  ...
  async escribir(valor, _anterior, opciones = {}) {
    const sb = obtenerCliente();
    // Se guarda solo el período que de verdad cambió (lo marca la pantalla
    // en opciones.periodo) — nunca los cuatro juntos: así escribir en
    // "diario" no pisa con una copia vieja lo que otra pestaña ya guardó en
    // "mensual" mientras esta seguía abierta.
    const periodos = opciones.periodo ? [opciones.periodo] : PERIODOS_BITACORA;
    for (const periodo of periodos) {
      const datos = valor[periodo] || { columns: [], rows: [], notes: "" };
      const { error } = await sb.from("bitacora_admin").update({
        columnas: datos.columns || [],
        filas: datos.rows || [],
        notas: datos.notes || "",
        actualizado_at: new Date().toISOString(),
        actualizado_por: opciones.idUsuario || null,
      }).eq("periodo", periodo);
      if (error) throw new Error(`No se pudo guardar "${periodo}": ${error.message}`);
    }
  },
};
...
/* Protección contra una baja masiva por accidente.

   Hasta agosto de 2026 una baja de producto se deducía en silencio: cualquier
   producto que estaba en la última copia conocida y no venía en la lista que
   se guardaba se tomaba como "lo sacaron del catálogo". El supuesto era que
   la pantalla siempre mandaba el catálogo COMPLETO — pero si por lo que sea
   mandaba una copia incompleta (quedó atrasada, una sincronización se saltó
   un producto que otra caja acababa de crear), el sistema no tenía forma de
   distinguir eso de una baja real, y apagaba en silencio todo lo que
   faltara. Así se dieron de baja solos dos productos en agosto de 2026 sin
   que nadie los hubiera tocado: sus códigos de barra dejaron de funcionar en
   la caja de un día para otro.

   Ahora una baja SOLO ocurre cuando la pantalla la pide de forma explícita
   (ver `opciones.idsAEliminar` en escribir() más abajo) — nunca por
   deducción a partir de lo que falte en la lista. Este tope se mantiene de
   todos modos como red de seguridad: una baja de verdad, hecha por una
   persona desde la pantalla, es siempre de a un producto por vez — no existe
   un "eliminar varios". Por eso, pasado este número en una sola llamada, se
   corta con un error en vez de aplicarlas: mejor avisar altiro que vaciar el
   catálogo sin que nadie se dé cuenta hasta días después. */
const TOPE_BAJAS_IMPLICITAS = 3;
function bajasSeguras(aEliminar) {
  if (aEliminar.length > TOPE_BAJAS_IMPLICITAS) {
    throw new Error(
      `Se iba a dar de baja ${aEliminar.length} productos de una sola vez ` +
      `(${aEliminar.slice(0, 5).map((p) => p.name || p.id).join(", ")}${aEliminar.length > 5 ? "…" : ""}), ` +
      `algo que nadie pide hacer así. Se canceló esa baja para no perder productos por error: ` +
...
export const productos = {
    ...
    if (!incremental) {
      const historial = await traerTodo(() =>
        sb.from("producto_precio_historial")
          .select("producto_id,fecha,costo,precio,omite_proteccion_anterior")
          .order("producto_id").order("fecha", { ascending: true })
      );
      const porProducto = new Map();
      for (const h of historial) {
        const lista = porProducto.get(h.producto_id) || [];
        // Si quien recibió o corrigió el precio confirmó a propósito que
        // aceptaba la pérdida (ver ReceivingView), esta entrada queda marcada
        // y findLastPriceDrop() la salta al buscar la última baja a proteger.
        lista.push({ date: h.fecha, cost: num(h.costo), price: num(h.precio), omitido: !!h.omite_proteccion_anterior });
  ...
  async escribir(actual, anterior, opciones = {}) {
    ...
    if (nuevos.length) {
      ...
      // sin terminar con el catálogo duplicado. La comparación es "a la
      // EAN-13" (ver normalizarCodigoParaComparar): un código que ya existe
      // con un cero adelante (o sin él) también cuenta como ya cargado, para
      // no terminar con dos fichas del mismo producto físico.
      const yaCargados = new Set(
        (await traerTodo(() => sb.from("producto").select("codigo_barras")))
          .map((x) => normalizarCodigoParaComparar(x.codigo_barras))
      );
      const filasNuevas = filas.filter(
        (f) => !yaCargados.has(normalizarCodigoParaComparar(f.codigo_barras))
      );
      ...
      await enParalelo(
        nuevos.filter((p) => idsNuevos.has(p.id) && p.priceApproval),
        (p) => crearAprobacion(sb, p, idUsuario)
      );
    }

    // --- Productos modificados ---
    // Las secciones NUEVAS que aparezcan entre los cambiados se resuelven
    // antes, todas juntas y una sola vez por nombre — igual que con los
    // productos nuevos más arriba — para no depender de que asegurarCategoria
    // se las arregle sola si dos productos de la misma tanda en paralelo
    // estrenaran la misma sección al mismo tiempo (se las arregla: reintenta
    // leyendo si el insert choca — pero mejor no necesitar esa carrera).
    const seccionesNuevas = new Map();
    for (const { antes, ahora } of cambiados) {
      if (antes.category !== ahora.category && !seccionesNuevas.has(ahora.category)) {
        seccionesNuevas.set(ahora.category, await asegurarCategoria(ahora.category));
      }
    }

    // Cada producto modificado se guarda de forma independiente del resto —
    // ninguno necesita esperar a que el anterior termine — así que se hacen
    // de a POR_TANDA_CONCURRENTE a la vez en vez de uno por uno en fila (ver
    // enParalelo): en una recepción con hartas líneas, esto es lo que hacía
    // que actualizar se sintiera lento.
    await enParalelo(cambiados, async ({ antes, ahora }) => {
      if (distintos(antes, ahora, CAMPOS_PRODUCTO)) {
        // OJO: acá NO se arma la fila entera con lo que "ahora" tenga para cada
        // campo. Antes se hacía así, y bastaba con que UN campo cambiara (el
        // stock, la sección, el acceso rápido) para que TODOS se reescribieran
        // con el valor que este mismo dispositivo tuviera en memoria — incluido
        // el precio. Si esa memoria venía atrasada (la pantalla llevaba un rato
        // en segundo plano, se saltó un ciclo de sincronización de 15s, dos cajas
        // editando casi al mismo tiempo), el precio que otra persona acababa de
        // corregir se volvía a pisar con el valor viejo, en silencio y sin dejar
        // rastro en el historial de precios (esta rama nunca llama a
        // pushPriceHistory). Así se perdían correcciones de precio hechas por un
        // administrador sin que nada en el sistema avisara.
        // Ahora se manda solo el campo que de verdad cambió EN ESTA PANTALLA
        // (comparando la propia copia anterior contra la propia copia actual):
        // lo que no cambió acá se deja tal cual esté en la base, así nunca se
        // pisa por accidente un valor que otro dispositivo dejó más al día.
        const fila = {};
        if (distintos(antes, ahora, ["barcode"])) fila.codigo_barras = ahora.barcode || antes.barcode;
        if (distintos(antes, ahora, ["name"])) fila.nombre = ahora.name;
        if (antes.category !== ahora.category) fila.categoria_id = seccionesNuevas.get(ahora.category);
        if (distintos(antes, ahora, ["price"])) fila.precio = num(ahora.price);
        if (distintos(antes, ahora, ["cost"])) fila.costo = num(ahora.cost);
        if (distintos(antes, ahora, ["minStock"])) fila.stock_minimo = num(ahora.minStock, 5);
        if (distintos(antes, ahora, ["supplierId"])) fila.proveedor_id = ahora.supplierId || null;
        if (distintos(antes, ahora, ["unitType"]) || distintos(antes, ahora, ["unitsPerKg"])) {
          fila.tipo_unidad = ahora.unitType === "peso" ? "peso" : "unidad";
          fila.unidades_por_kg = ahora.unitType === "peso" ? null : (ahora.unitsPerKg ?? null);
        }
        if (distintos(antes, ahora, ["quickAccess"])) fila.acceso_rapido = !!ahora.quickAccess;
    ...
    // Solo se retira lo que la propia pantalla pide por su id, nunca lo que
    // "falte" en la lista guardada (ver la nota junto a bajasSeguras): eso es
    // lo que evita que un producto se apague solo por una copia atrasada.
    const aEliminar = opciones.sinBajas ? [] : bajasSeguras(opciones.idsAEliminar || []);
    for (const p of aEliminar) {
      const { error } = await sb.from("producto").update({ activo: false }).eq("id", p.id);
      if (error) throw new Error(`No se pudo retirar "${p.name || p.id}": ${error.message}`);
    }
  },
};
...
/* Para tareas independientes entre sí —un producto no necesita esperar a que
   termine el anterior para guardarse— pero que antes se hacían de a una, en
   un for-of con await adentro: cada vuelta esperaba su propia ida y vuelta a
   la red aunque no dependiera para nada de la anterior, y esas esperas se
   sumaban en vez de superponerse. Eso era lo que hacía tan lenta una
   recepción con hartas líneas: 30 productos modificados eran 30 (o 60,
   contando el kárdex) idas y vueltas en fila. De a POR_TANDA a la vez, la
   espera total es la de una sola vuelta de la tanda más lenta, no la suma de
   todas.

   El tamaño de tanda es a ojo: bastante para que no quede nada por delante
   esperando en fila sin necesidad, no tanto como para mandarle a la base 100
   consultas en el mismo instante si algún día una recepción viene enorme. */
const POR_TANDA_CONCURRENTE = 8;
async function enParalelo(items, tarea) {
  for (let i = 0; i < items.length; i += POR_TANDA_CONCURRENTE) {
    await Promise.all(items.slice(i, i + POR_TANDA_CONCURRENTE).map(tarea));
  }
}
...
export const ofertas = {
  async leer(opciones = {}) {
    const sb = obtenerCliente();
    const [carpetas, miembros, tramos, imagenes] = await Promise.all([
      ...
      // Imagen promocional (migración 0034): se trae junto con la carpeta
      // para que la lista pueda mostrar de una que ya tiene una armada, sin
      // una consulta aparte por cada oferta.
      traerTodo(() => sb.from("oferta_imagen").select("oferta_id,storage_path")),
    ]);
    ...
    // Bucket público: la URL se arma sola con la ruta, sin pedirle nada más
    // a la base (a diferencia de las fotos de facturas, que van a un bucket
    // privado y necesitan un enlace firmado que hay que ir a buscar).
    const archivos = obtenerClienteArchivos();
    const imagenPorOferta = new Map();
    for (const im of imagenes) {
      const { data: pub } = archivos.storage.from(BUCKET_PUBLICO).getPublicUrl(im.storage_path);
      if (pub?.publicUrl) imagenPorOferta.set(im.oferta_id, pub.publicUrl);
    }
    ...
    return carpetas.map((o) => ({
      ...
      imageUrl: imagenPorOferta.get(o.id) || null,
    }));
  },
...
/* =====================================================================
   product-groups  ↔  galpon.producto_grupo (+ producto_grupo_miembro)

   "Producto unificado" (migración 0033): varias marcas/proveedores de un
   mismo tipo de producto (ej. jamonada) que se reciben y cuestan cada una
   por separado, pero se venden todas bajo un solo nombre y a un solo
   precio. Mismo patrón que "ofertas" (carpeta + miembros), pero sin tramos:
   acá solo hace falta la lista de miembros — cuál marca se descuenta al
   vender y el costo promedio se resuelven en la pantalla de venta, con el
   catálogo que ya está cargado, no acá. */
export const gruposProducto = {
  async leer(opciones = {}) {
    const sb = obtenerCliente();
    const [grupos, miembros] = await Promise.all([
      traerTodo(() => sb.from("producto_grupo").select("*").eq("activo", true).order("nombre")),
      traerTodo(() => sb.from("producto_grupo_miembro").select("grupo_id,producto_id")),
    ]);
    const miembrosPorGrupo = new Map();
    for (const m of miembros) {
      const lista = miembrosPorGrupo.get(m.grupo_id) || [];
      lista.push(m.producto_id);
      miembrosPorGrupo.set(m.grupo_id, lista);
    }
    return grupos.map((g) => ({
      id: g.id,
      name: g.nombre,
      price: num(g.precio),
      unitType: g.tipo_unidad,
      memberIds: miembrosPorGrupo.get(g.id) || [],
      createdAt: g.creado_at,
    }));
  },
  ...
  async escribir(actual, anterior, opciones = {}) {
    ...
    async function reemplazarMiembros(grupoId, memberIds) {
      const { data: existentes, error: errLeer } = await sb
        .from("producto_grupo_miembro").select("producto_id").eq("grupo_id", grupoId);
      if (errLeer) throw new Error(`Grupo de productos: ${errLeer.message}`);
      const actuales = new Set((memberIds || []).filter(Boolean));
      const previos = new Set((existentes || []).map((x) => x.producto_id));
      const aQuitar = [...previos].filter((id) => !actuales.has(id));
      const aAgregar = [...actuales].filter((id) => !previos.has(id));
      if (aQuitar.length) {
        const { error } = await sb.from("producto_grupo_miembro")
          .delete().eq("grupo_id", grupoId).in("producto_id", aQuitar);
        if (error) throw new Error(`Grupo de productos: ${error.message}`);
      }
      if (aAgregar.length) {
        const { error } = await sb.from("producto_grupo_miembro")
          .insert(aAgregar.map((producto_id) => ({ grupo_id: grupoId, producto_id })));
        // Un producto ya asignado a otro grupo choca con la llave única de
        // producto_grupo_miembro.producto_id: se avisa tal cual, para que se
        // sepa que hay que sacarlo del otro grupo primero.
        if (error) throw new Error(/unique|duplicat/i.test(error.message)
          ? "uno de los productos elegidos ya pertenece a otro grupo"
          : error.message);
      }
    }

    for (const g of nuevos) {
      const { error } = await sb.from("producto_grupo").insert({
        id: g.id, nombre: g.name, precio: num(g.price), tipo_unidad: g.unitType || "peso",
      });
      if (error) throw new Error(`Grupo de productos "${g.name}": ${error.message}`);
      await reemplazarMiembros(g.id, g.memberIds);
    }

    for (const { antes, ahora } of cambiados) {
      if (antes.name !== ahora.name || antes.price !== ahora.price || antes.unitType !== ahora.unitType) {
        const { error } = await sb.from("producto_grupo").update({
          nombre: ahora.name, precio: num(ahora.price), tipo_unidad: ahora.unitType || "peso",
        }).eq("id", ahora.id);
        if (error) throw new Error(`Grupo de productos "${ahora.name}": ${error.message}`);
      }
      if (distintaListaIds(antes.memberIds, ahora.memberIds)) {
        await reemplazarMiembros(ahora.id, ahora.memberIds);
      }
    }

    for (const g of (opciones.sinBajas ? [] : eliminados)) {
      const { error } = await sb.from("producto_grupo").update({ activo: false }).eq("id", g.id);
      if (error) throw new Error(`No se pudo desactivar el grupo "${g.name}": ${error.message}`);
      // Se sueltan los miembros acá mismo: si no, la llave única de
      // producto_grupo_miembro.producto_id los dejaría "atrapados" —ni
      // siquiera se ven en la lista, porque el grupo ya está desactivado—
      // sin poder entrar a un grupo nuevo ni volver a este si se recrea.
      const { error: eMiembros } = await sb.from("producto_grupo_miembro").delete().eq("grupo_id", g.id);
      if (eMiembros) throw new Error(`No se pudieron soltar los productos del grupo "${g.name}": ${eMiembros.message}`);
    }
  },
};
...
/* Todos los códigos de barra del catálogo —activos e inactivos— para que una
   pantalla pueda avisar ANTES de guardar que un código ya lo tiene otro
   producto, en vez de enterarse recién cuando la base rechaza el índice
   único (o, peor, sin enterarse nunca: ver normalizarCodigoParaComparar en
   lib/codigos-barra.js sobre el mismo código escrito con o sin un cero
   adelante). El catálogo que ya tiene la pantalla no sirve para esto porque
   solo trae los productos activos.

   Se piden solo estas cuatro columnas —nunca todo el producto— porque esto
   se llama antes de confirmar una recepción o crear un producto, con la
   persona esperando. */
export async function todosLosCodigosDeBarra() {
  const sb = obtenerCliente();
  const filas = await traerTodo(() =>
    sb.from("producto").select("id,nombre,codigo_barras,activo")
  );
  return filas.map((f) => ({
    id: f.id, name: f.nombre, barcode: f.codigo_barras || "", active: !!f.activo,
  }));
}

/* Productos dados de baja, para el panel de Inventario donde se pueden
   reactivar. Trae stock y precio además de nombre/código —no solo lo que
   pide todosLosCodigosDeBarra para el chequeo de duplicados— porque acá el
   objetivo es que quien administra pueda ver algo útil para decidir si la
   baja fue un error, sin tener que pedir que alguien la revierta a mano en
   la base (que es como se resolvió cada una de estas hasta ahora). */
export async function productosDadosDeBaja() {
  const sb = obtenerCliente();
  const filas = await traerTodo(() =>
    sb.from("producto").select("id,nombre,codigo_barras,stock,precio,actualizado_at")
      .eq("activo", false)
      .order("actualizado_at", { ascending: false })
  );
  return filas.map((f) => ({
    id: f.id, name: f.nombre, barcode: f.codigo_barras || "",
    stock: num(f.stock), price: num(f.precio), updatedAt: f.actualizado_at,
  }));
}

/* Reactivar un producto dado de baja: el único camino de vuelta que existe
   hoy, aparte de entrar directo a la base. No pasa por productos.escribir()
   —no hay ningún otro campo que cambiar, es activo=true y nada más— así que
   no tiene sentido mandar el catálogo completo solo para esto. */
export async function reactivarProducto(id) {
  const sb = obtenerCliente();
  const { error } = await sb.from("producto").update({ activo: true }).eq("id", id);
