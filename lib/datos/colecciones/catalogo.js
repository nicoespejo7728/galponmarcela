...
export async function unificarCategorias(idDestino, idsOrigen) {
  ...
  const { data, error } = await sb
    ...
    .in("categoria_id", origenes)
    .select("id");
  if (error) throw new Error(`No se pudieron mover los productos: ${error.message}`);

  const { error: errorCat } = await sb
    .from("categoria").update({ activa: false }).in("id", origenes);
  if (errorCat) throw new Error(`No se pudieron desactivar las categorías: ${errorCat.message}`);
  ...
  return { movidos: (data || []).length, desactivadas: origenes.length };
}
