...
export async function subirVentaCompleta({ venta, idUsuario }) {
  ...
  const { data: creada, error } = await sb.from("venta")
    .insert(filaVenta).select("id,numero_boleta").single();
  if (error && !yaExistia(error)) throw new Error(`Boleta: ${error.message}`);
  if (creada?.numero_boleta) numeroReal = creada.numero_boleta;

  // El detalle y el kárdex solo se escriben si la boleta se creó recién. Si ya
  // existía, esta venta se subió antes y repetir el kárdex descontaría el
  // stock dos veces — que es exactamente lo que dejó doce productos con el
  // stock al doble durante la unificación.
  if (!error) {
    const lineas = (v.items || []).map((it) => ({
      venta_id: v.id,
      producto_id: it.productId || null,
  ...
  return { numeroBoleta: numeroReal, yaEstaba: !!error };
}
