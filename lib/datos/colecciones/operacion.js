...
export const facturas = {
  ...
  async escribir(actual, anterior, opciones = {}) {
    const sb = obtenerCliente();
    // Antes esto solo insertaba lo nuevo: una recepción, una vez guardada,
    // no se podía corregir — un número de boleta mal tecleado o un total
    // errado quedaban así para siempre. Ahora también se detectan las
    // editadas (ver Recepción → cargar una recepción existente para
    // editarla) — el borrado no se contempla: una recepción no se elimina
    // sola, sólo se corrige.
    const { nuevos, cambiados } = diferencias(anterior, actual);
    const idUsuario = opciones.idUsuario || null;

    function filaDesde(f) {
      ...
      fila.monto_informado = f.statedTotal != null && f.statedTotal !== "" ? num(f.statedTotal) : null;
      fila.fecha_pago_prevista = f.duePaymentDate || null;
      return fila;
    }
    ...
    // --- Recepciones editadas ---
    // A propósito NO se toca el proveedor acá (ni su nombre): cambiar a qué
    // proveedor pertenece una recepción ya guardada arrastra demasiado
    // (líneas de compra, cargo en el libro de crédito) — si el proveedor
    // está mal, se anula esa recepción y se registra una nueva.
    const CAMPOS_FACTURA_EDITABLES = [
      "refNumber", "totalNet", "documentType", "noDocument", "reason",
      "paymentMethod", "statedTotal", "duePaymentDate", "date",
    ];
    for (const { antes, ahora } of cambiados) {
      const cambioDesglose = JSON.stringify(antes.paymentBreakdown || null) !== JSON.stringify(ahora.paymentBreakdown || null);
      if (!distintos(antes, ahora, CAMPOS_FACTURA_EDITABLES) && !cambioDesglose) continue;
      const { error } = await sb.from("factura_compra").update(filaDesde(ahora)).eq("id", ahora.id);
      if (error) throw new Error(`No se pudo corregir la recepción: ${error.message}`);
    }
  },
};
...
export const lineasDeCompra = {
  ...
  async escribir(actual, anterior, opciones = {}) {
    const sb = obtenerCliente();
    // Antes esto solo insertaba: una línea de compra, una vez guardada, no se
    // podía corregir ni quitar. Ahora una recepción ya guardada se puede volver
    // a abrir (ver Recepción → cargar por número de boleta) y ahí sí hace falta
    // sumar, restar o sacar productos de la lista original — de ahí cambiados
    // y eliminados. El ajuste de stock que corresponde a estos cambios NO se
    // hace acá: lo calcula quien llama (ReceivingView), pasando la lista de
    // productos ya corregida por el mecanismo normal de products-catalog.
    const { nuevos, cambiados, eliminados } = diferencias(anterior, actual);
    const idUsuario = opciones.idUsuario || null;
    ...
    // --- Líneas de una recepción ya guardada, corregidas ---
    const CAMPOS_LINEA_EDITABLES = ["qty", "netCost", "productId", "productName"];
    for (const { antes, ahora } of cambiados) {
      if (!distintos(antes, ahora, CAMPOS_LINEA_EDITABLES)) continue;
      const { error } = await sb.from("compra_detalle").update({
        producto_id: ahora.productId || null,
        nombre_producto: ahora.productName || "",
        cantidad: Math.abs(num(ahora.qty)) || 1,
        costo_neto_unitario: num(ahora.netCost),
      }).eq("id", ahora.id);
      if (error) throw new Error(`No se pudo corregir la línea de compra: ${error.message}`);
    }

    // --- Líneas sacadas de una recepción ya guardada ---
    for (const l of eliminados) {
      const { error } = await sb.from("compra_detalle").delete().eq("id", l.id);
      if (error) throw new Error(`No se pudo quitar la línea de compra: ${error.message}`);
    }
  },
};
