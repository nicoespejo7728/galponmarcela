import { obtenerCliente, obtenerClienteArchivos, BUCKET_FACTURAS, BUCKET_PUBLICO } from "@/lib/supabase/cliente";
...
/* Imagen promocional de una oferta (carpeta de "N por $X"), armada en el
   navegador con <canvas> (ver lib/promo-oferta.js) a partir de su nombre,
   sus tramos y —si se adjuntaron— fotos de los productos que participan.

   A diferencia de las fotos de facturas/gastos, esta imagen va al bucket
   PÚBLICO (el mismo del logo del negocio): está pensada para compartirse
   afuera del sistema —WhatsApp, redes— así que necesita un enlace que no
   venza como el firmado de galpon-facturas. Por eso getPublicUrl en vez de
   createSignedUrl, y no hace falta pedirle nada a la base para poder
   mostrarla apenas se sube: el enlace se arma solo con la ruta. */
export async function guardarImagenOferta(ofertaId, dataUrl, mediaType = "image/jpeg") {
  if (!dataUrl) return null;
  const archivos = obtenerClienteArchivos();
  const sb = obtenerCliente();
  const ext = EXTENSIONES[mediaType] || "jpg";
  const ruta = `ofertas/${ofertaId}.${ext}`;
  const contenido = aBinario(dataUrl);

  const { error: errSubida } = await archivos.storage
    .from(BUCKET_PUBLICO)
    .upload(ruta, contenido, { contentType: mediaType, upsert: true });
  if (errSubida) throw new Error(`No se pudo guardar la imagen promocional: ${errSubida.message}`);

  const { error: errFila } = await sb.from("oferta_imagen").upsert(
    { oferta_id: ofertaId, storage_path: ruta, tipo_mime: mediaType },
    { onConflict: "oferta_id" }
  );
  if (errFila) throw new Error(`No se pudo registrar la imagen promocional: ${errFila.message}`);

  const { data: pub } = archivos.storage.from(BUCKET_PUBLICO).getPublicUrl(ruta);
  return pub?.publicUrl || null;
}

export async function quitarImagenOferta(ofertaId) {
  const archivos = obtenerClienteArchivos();
  const sb = obtenerCliente();
  const { data: fila } = await sb
    .from("oferta_imagen").select("storage_path").eq("oferta_id", ofertaId).maybeSingle();
  if (fila?.storage_path) {
    await archivos.storage.from(BUCKET_PUBLICO).remove([fila.storage_path]);
  }
  const { error } = await sb.from("oferta_imagen").delete().eq("oferta_id", ofertaId);
  if (error) throw new Error(`No se pudo quitar la imagen promocional: ${error.message}`);
}
