...
export const A_METODO_PAGO = {
  ...
  // Se agregó con el fiado (migración 0012). La venta se cobra "después":
  // no es dinero que haya entrado a la caja todavía, así que se trata
  // distinto del resto en el libro de caja (ver checkout() en el POS).
  Fiado: "fiado",
};
export const DESDE_METODO_PAGO = invertir(A_METODO_PAGO);

/* --- Método de pago a proveedores (recepción de mercadería, migración 0014) ---
   Enum aparte de A_METODO_PAGO: acá "Crédito con el proveedor" significa
   que EL GALPÓN queda debiendo esa recepción, no que se pagó con tarjeta
   de crédito como en el POS — por eso no comparten el mismo enum ni el
   mismo mapa, aunque el nombre "crédito" se parezca. */
export const A_METODO_PAGO_PROVEEDOR = {
  Efectivo: "efectivo",
  Transferencia: "transferencia",
  "Crédito con el proveedor": "credito",
};
export const DESDE_METODO_PAGO_PROVEEDOR = invertir(A_METODO_PAGO_PROVEEDOR);

/* --- Categoría de movimiento del libro de caja ---
   Las de "gastos del local" y "Pago a proveedor" se agregaron en la
   migración 0011, junto con la sección "Pagos y gastos" de Finanzas. */
export const A_CATEGORIA_MOVIMIENTO = {
  ...
  Arriendo: "arriendo",
  "Bencina y combustible": "bencina",
  Gas: "gas",
  "Insumos y aseo": "insumos_aseo",
  "Mantención y reparaciones": "mantencion",
  "Impuestos y contribuciones": "impuestos",
  "Otro gasto del local": "otro_gasto_local",
  // No es una categoría elegible al crear un movimiento a mano: la genera
  // el propio sistema cuando un cliente abona su deuda de fiado (ver
  // registerPayment en ClientesView, migración 0012).
  "Abono de fiado": "abono_fiado",
