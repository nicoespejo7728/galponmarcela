...
/* Categoría de un producto para esta hoja: la que tenga, o el cajón de los
   que no tienen ninguna asignada — para que ese puñado también aparezca
   agrupado en vez de desperdigado según cómo haya quedado el orden de
   entrada. */
function seccionDe(p) {
  return p?.category || SIN_SECCION;
}

/* Agrupa por categoría (alfabético, sin distinguir tildes ni mayúsculas
   para el orden pero conservando el nombre tal cual para mostrarlo) y
   ordena los productos de cada una por nombre — así se ubican con el dedo
   igual que antes, ahora categoría por categoría. */
function agruparPorCategoria(productos) {
  const porCategoria = new Map();
  for (const p of productos || []) {
    const cat = seccionDe(p);
    if (!porCategoria.has(cat)) porCategoria.set(cat, []);
    porCategoria.get(cat).push(p);
  }
  const ordenadas = [...porCategoria.keys()].sort((a, b) => a.localeCompare(b, "es"));
  for (const cat of ordenadas) {
    porCategoria.get(cat).sort((a, b) =>
      String(a.name || "").localeCompare(String(b.name || ""), "es"));
  }
  return { porCategoria, ordenadas };
}

/* Reparte los productos en páginas de CARTA_POR_HOJA celdas, categoría por
   categoría: cada una intenta empezar hoja nueva, salvo que quepa entera en
   lo que sobró de la anterior — así una categoría grande no se corta a la
   mitad sin necesidad, pero una chica no gasta una hoja completa para ella
   sola. Si una categoría es más grande que una hoja, ocupa las que hagan
   falta y la última —la que queda a medio llenar— sigue abierta para que
   la próxima categoría chica se sume ahí.

   Se expone aparte de hojaCartaDeCodigos para poder mostrar en pantalla,
   antes de imprimir, en cuántas hojas va a salir y qué categoría cae en
   cada una — sin tener que generar el HTML completo solo para contarlas. */
export function empaquetarHojaCarta(productos) {
  const { porCategoria, ordenadas } = agruparPorCategoria(productos);

  const paginas = [];
  let actual = { celdas: [], categorias: [] };
  function cerrarPagina() {
    if (actual.celdas.length > 0) paginas.push(actual);
    actual = { celdas: [], categorias: [] };
  }

  for (const cat of ordenadas) {
    const items = porCategoria.get(cat);
    const libres = CARTA_POR_HOJA - actual.celdas.length;
    // No cabe entera en lo que queda de la hoja actual: se cierra (si tenía
    // algo) y esta categoría parte hoja limpia.
    if (items.length > libres && actual.celdas.length > 0) cerrarPagina();

    let i = 0;
    while (i < items.length) {
      const libresAhora = CARTA_POR_HOJA - actual.celdas.length;
      const tanda = items.slice(i, i + libresAhora);
      actual.celdas.push(...tanda);
      if (!actual.categorias.includes(cat)) actual.categorias.push(cat);
      i += tanda.length;
      if (i < items.length) cerrarPagina();   // la categoría sigue: hoja nueva, llena
    }
  }
  cerrarPagina();
  return paginas;
...
/* En cuántas hojas cae una lista de productos, y qué categoría(s) trae cada
   una. Se expone para poder decirlo en pantalla antes de abrir la ventana de
   impresión. */
export function hojasQueSalen(productos) {
  return empaquetarHojaCarta(productos).length;
}

export function hojaCartaDeCodigos(productos, settings, hoy = new Date()) {
  const paginas = empaquetarHojaCarta(productos);
  ...
  const cuerpo = paginas.map((pagina, n) => `
    <section class="pagina">
      <header class="cabecera">
        <span><strong>${negocio}</strong> · códigos internos · ${escaparHtml(pagina.categorias.join(" + "))}</span>
      ...
      <div class="grilla">
        ...
        ${Array.from({ length: CARTA_POR_HOJA - pagina.celdas.length },
...
<body>
  <p class="aviso">${(productos || []).length} producto(s) en ${paginas.length} hoja(s) tamaño carta, organizados por categoría. Imprime al 100%, sin "ajustar a la página": si se achica, las barras se angostan y la pistola deja de leerlas.</p>
  ${cuerpo}
